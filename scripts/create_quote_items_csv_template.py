"""Create an empty strict quote-items CSV template outside Git."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create an empty strict quote-items CSV template outside Git."
    )
    parser.add_argument(
        "--output",
        required=True,
        type=Path,
        help="New .csv path outside the Git project",
    )
...
def format_report(result: TemplateResult) -> str:
    return "\n".join(
        [
            "QUOTE_ITEMS_CSV_TEMPLATE_REPORT",

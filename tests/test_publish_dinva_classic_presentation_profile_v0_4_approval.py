...
from test_dinva_v0_4_native_visual import native_successor


@pytest.fixture
def case(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    _, payload, _, _, _ = native_successor(tmp_path / "evidence", monkeypatch)
    monkeypatch.syspath_prepend(str(ROOT / "scripts"))
    publisher = load_file(
        "v04_approval_tests",
        ROOT / "scripts/publish_dinva_classic_presentation_profile_v0_4_approval.py",
    )
...
def test_content_bound_copy_and_second_publication(case: dict[str, Any]) -> None:
    p = case["p"]
    ...
    with pytest.raises(p.ProfileV04ApprovalError, match="already exists"):
        publish(case)
...
def test_approval_rejections(case: dict[str, Any], kind: str) -> None:
    ...
    if kind == "wrong_sha":
        ...
        case["token"] = case["token"].replace("V0_4", "V0_3")
    elif kind == "token_sha":
    ...
    with pytest.raises(p.ProfileV04ApprovalError):
        publish(case)
    if kind == "collision":
...
@pytest.mark.parametrize(
    "kind",
    ["extra_binding", "binding_sha", "schema", "approval_state", "fingerprint"],
)
def test_subject_structure_fail_closed(
    ...
    with pytest.raises(case["p"].ProfileV04ApprovalError):
        publish(case)
    assert not case["output"].parent.exists()
...
def test_atomic_publication_races_are_fail_closed(
    ...
    with pytest.raises((OSError, p.ProfileV04ApprovalError)):
        publish(case)
    if kind == "collision":
...
def test_inside_git_output_rejected(case: dict[str, Any]) -> None:
    case["output"] = ROOT / "v04-approved.synthetic" / case["p"].OUTPUT_FILENAME
    with pytest.raises(case["p"].ProfileV04ApprovalError, match="outside Git"):
        publish(case)
    assert not case["output"].parent.exists()


def test_json_only_capability_has_no_renderer_or_downstream() -> None:
    path = ROOT / "scripts/publish_dinva_classic_presentation_profile_v0_4_approval.py"
